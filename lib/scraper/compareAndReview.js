...
const getMatchingOrders = function (orders, transactions, toleranceDays = 5) {
  let matches = {
    likely: {},
    unsure: []
  }
  for (let i in transactions) {
    ...
    let inRangeOrders = possibleOrders.filter(order => Math.abs(differenceInDays(new Date(order.date), new Date(transaction.date))) < toleranceDays)
...
const getMatchingTransactions = function (orders, transactions, toleranceDays = 5) {
  let matches = {
    likely: {},
    unsure: [],
    none: []
  ...
  for (let i in orders) {
    let order2 = orders[i]
    let possibleTransactions = transactions.filter(t => t.amount == order2.total)
    if (possibleTransactions.length == 0) {
      matches.none.push({
        order: order2,
        transactions: [],
        reason: "no transactions matching that total found"
      })
      continue
    }

    let inRangeTrans = possibleTransactions.filter(trans => Math.abs(differenceInDays(new Date(trans.date), new Date(order2.date))) < toleranceDays)

    if (inRangeTrans.length == 1) {
      matches.likely[order2.orderId] = {
        order: order2,
        transaction: inRangeTrans[0]
      }
    } else if (inRangeTrans.length > 1) {
      matches.unsure.push({
        order: order2,
        transactions: inRangeTrans,
        reason: "too many likely matches"
      })
    } else if (possibleTransactions.length == 1) {
      matches.unsure.push({
        order: order2,
        transactions: possibleTransactions[0],
        reason: "Transaction is out of normal date range!"
      })
    } else {
      matches.unsure.push({
        order: order2,
